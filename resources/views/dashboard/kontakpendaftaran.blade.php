            ...
            @foreach($pesan as $pesans)
                <tr>
                    ...
                    <td>
                      ...
                      <a href="{{ route('admin-kontakpendafataran.show', $pesans->id) }}" class="btn btn-warning">Balas</a>
                      <form action="{{ route('admin-kontakpendafataran.destroy',$pesans->id) }}" method="POST" type="button" class="btn btn-danger p-0" onsubmit="return confirm('Yakin ingin menghapus?')">
                        @csrf
                        @method('DELETE')
